...
The “Instances” column in Table 1 represents the number of usable artwork records that we successfully collected. The “Schema” column indicates whether the data source are using a formal structured ontology or schema (e.g., RDF/Linked Data using standards like CIDOC-CRM or schema.org, marked “Yes”), enabling direct semantic queries (e.g., via SPARQL) and automated metadata mapping, or only unstructured/flat formats (e.g., CSV/JSON dumps, marked “No”).
The “Attrs.” column refers to the total number of distinct metadata attributes extracted and unified for each artwork, including fields such as title, creator (artist), style, date/period, medium/material, dimensions, description, location, genre, and others relevant to art analysis.
The “Copyright” column specifies the usage restrictions and license types for each dataset, which directly determined our data collection and filtering pipeline—ensuring compliance by collecting only permissible instances.
